...
static const size_t ANIM_MAX_IMAGE_BYTES = 24u * 1024 * 1024;

// Стеля на сторону кадру анімованої картинки.
//
// Гіфки з чату Twitch приходять такими, якими лежать на Giphy, — 480 пікселів
// у стороні й більше. У рядку вони займають десь 90 пікселів заввишки, тобто
// пʼять шостих кожного кадру ми розібрали б, поклали в памʼять і викинули при
// малюванні. Один такий емоут на 15 кадрів — 13 МБ, а це більше за стелю вище:
// анімація просто не вмикалася, і гіфка стояла нерухомо. Тепер кадри
// зменшуються до розумного, а «природний» розмір (той, який бачить CSS)
// лишається справжнім — так само, як для SVG.
static const int ANIM_MAX_SIDE = 256;
...
private:
    ...
    // Зменшити кадри анімованої картинки до ANIM_MAX_SIDE (див. там же чому).
    void shrink(Image* img) const;
