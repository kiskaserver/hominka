...
public:
    ...
    // header — смужка ввімкнена в налаштуваннях; тоді вона видно завжди.
    // Замкнене вікно питає окреме налаштування (look.lock_header).
    bool visible(const Look& look, bool header = false) const;
    ...
    // ані налаштувань.
    // header — смужку видно завжди, як заголовок вікна. Вимкнена — на екрані
    // лишаються самі повідомлення, а керування з'являється під курсором.
    // update — є оновлення: шестерня отримує крапку. Розповідати про це
    // вікном посеред ефіру не можна, а не сказати зовсім — означає, що людина
    // дізнається про виправлення через тиждень.
