...
class TopMostKeeper:
    """Тримає наше вікно зверху — але тільки коли попереду зʼявився хтось новий.

    Смикати SetWindowPos щосекунди без потреби не можна: у старих іграх це саме
    й дає мерехтіння, на яке скаржаться користувачі оверлеїв. Тому запамʼятовуємо
    останнє вікно переднього плану і реагуємо лише на зміну.
    """
