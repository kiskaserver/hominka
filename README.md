# Svitix — чат поверх гри, невидимий для OBS

<sub>автор: Mykyta Vinnyk · версія 1.0.0</sub>
...
Скинь йому **всю теку `dist\Svitix\`** (запакуй у zip). Усередині він
запускає **`Svitix.exe`** — Python не потрібен. Далі: **⚙ → Посилання на
...
1. Запусти **`build.bat`** (двічі клікни) — один раз збере `Svitix.exe`
   (кілька хвилин).
2. Далі запускай **`dist\Svitix\Svitix.exe`** (можна зробити ярлик на
