...
- **Посилання на чат** — за замовчуванням твій чат (`stream.svitix.com`).
  Встав сюди **посилання на YouTube** (на трансляцію `watch?v=…`, `youtu.be/…`
  або готове `live_chat`) — програма сама зробить popout-чат і накладе **гарний
  прозорий стиль** (м'які рожево-фіолетові плашки, без зайвого інтерфейсу
  YouTube). Порожньо або своя сторінка — показуємо **як є, без змін**.
...
запускає **`Hominka.exe`** — Python не потрібен. Далі: **⚙ → Посилання на
чат → встав лінк своєї YouTube-трансляції → OK**. Налаштування збережуться в
`config.json` поряд з .exe.
