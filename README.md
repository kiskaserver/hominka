...
У теці видно два файли: **`Hominka.exe`** і `config.json` (ваші налаштування —
його можна відкривати й правити). Службове приховано атрибутом «прихований»:

- `_internal/` — потрухи збірки (~500 МБ);
- `profile/` — кеш браузера.

Запакувати все в один .exe не вийшло: така збірка розпаковує **583 МБ у
тимчасову теку при кожному запуску**, і вікно так і не з'являється. Звичайна
збірка показує чат за ~1.4 с, тож лишили її, просто прибрали зайве з очей.
