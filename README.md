...
<sub>назва — від укр. «гомін» (гомін голосів у чаті) · автор: Mykyta Vinnyk · версія 1.1.0</sub>
...
## Вхід у YouTube (реакції та поле вводу)

Анонімному глядачеві YouTube **не показує ні панелі реакцій, ні поля вводу** —
унизу чату замість них напис «Sign in to chat». Тому:

**⚙ → «Увійти в YouTube»** → звичайний вхід у Google → повертає на чат.

Вхід зберігається в теці `profile/` поряд із програмою й переживає перезапуск,
тож робиться один раз. Панель реакцій програма переносить у власний куточок
унизу праворуч — інакше її ховає той самий стиль, що прибирає поле вводу.

> Якщо Google відмовляється впускати («цей браузер може бути небезпечним») —
> це його захист від вбудованих браузерів. Тоді реакцій не буде: показати їх
> без входу неможливо, вони живуть на боці YouTube.
